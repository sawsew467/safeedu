...
const FrameChat = ({ content, role, isLoading, isEnd }: TypeChat & { isLoading: boolean, isEnd: boolean }) => {
    if (isEnd && isLoading)
        return (
            <View
                style={[
        ...
        >
            <View style={styles.container_content}>
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }: { item: TypeChat, index: number }) => <FrameChat {...item} isLoading={loading} isEnd={index + 1 === chatData.length && item.role === "chatbot"} />}
                />
            </View>
        </HeaderShown>
    );
